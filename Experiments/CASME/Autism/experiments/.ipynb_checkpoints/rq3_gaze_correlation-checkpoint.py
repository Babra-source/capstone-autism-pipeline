"""
RQ3: Gaze Pattern ↔ Emotional State Correlation Analysis
=========================================================

Research Question:
    What is the correlation between gaze patterns (eye-region activations)
    and emotional states in autistic students?

This module answers RQ3 by:

1.  Feature extraction
        Run each trained eye model over the test set → collect (N, D) matrix
        of gaze feature vectors, paired with emotion labels.

2.  Pearson + Spearman correlation
        For each feature dimension d, compute Pearson r and Spearman r between
        the d-th feature value and a one-vs-rest emotion indicator per class.
        Produces a (D, C) correlation matrix → visualised as a heatmap.

3.  Top gaze features per emotion class
        Rank feature dimensions by absolute Pearson r magnitude.
        Bar charts of the top-20 most emotion-discriminative gaze dims.


Outputs saved to  results/rq3_gaze_correlation/ :
    gaze_tsne_<model>.png
    gaze_pearson_heatmap_<model>.png
    gaze_top_features_<model>.png
    gaze_class_profiles_<model>.png
    gaze_correlation_summary_<model>.csv
    rq3_summary_<model>.json
"""
